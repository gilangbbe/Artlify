...
| Item                                                                            | Status        | Notes                                                                              |
| ------------------------------------------------------------------------------- | ------------- | ---------------------------------------------------------------------------------- |
| **Branch `particles` — silhouette-as-swarm + trails + audio reactivity + body-bursting shockwaves + negative-camera flash boxes + ASCII-dither overlay with sonar ring** | build green, untested on hardware | Dark bg, mask-attractive force, curl-noise drift, alpha gated by silhouette. Trail accumulator (ping-pong + decay) leaves fluid wakes. AudioReactor (AVAudioEngine + vDSP FFT, low/mid/high + level + pan + transient) modulates flow, glow, hue, fires a stereo-panned shockwave from `bodyCenter` (avg of hip joints) with relaxed mask gate so particles burst OUT through the silhouette. Random negative-camera windows (`NegativeBoxes.metal`) flash around body joints at ~9 Hz, showing the live feed colour-inverted on the dark canvas. ASCII overlay (`Ascii.metal` + glyph-strip atlas) re-renders the camera-through-silhouette as monospaced dot-matrix glyphs sparse→dense by luminance, with a single audio-triggered ring expanding outward from the body anchor on each transient. New mic entitlement + Info.plist key. |
